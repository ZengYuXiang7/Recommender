...
class NeuCF(torch.nn.Module):
    def __init__(self, args):
        ...
        self.dropout = 0.10
        self.rank = args.rank
        self.rank_gmf = args.rank
        self.rank_mlp = args.rank * (2 ** (self.num_layers - 1))
        self.embed_user_GMF = torch.nn.Embedding(162541 + 1, self.rank_gmf)
        self.embed_user_MLP = torch.nn.Embedding(162541 + 1, self.rank_mlp)
        self.embed_item_GMF = torch.nn.Embedding(59047 + 1, self.rank_gmf)
        self.embed_item_MLP = torch.nn.Embedding(59047 + 1, self.rank_mlp)
